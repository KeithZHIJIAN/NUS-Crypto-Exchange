Instructions to build and install hiredis & redis++
...
cd hiredis
make PREFIX=\usr
make PREFIX=\usr install  //If you get a "permission denied", use sudo make PREFIX=\usr install
cd ..
```
...
make
make install  //If you get a "permission denied", use sudo make install
cd ..
```
...
redis-cli
//you will see the host and port
```
Run test, fill in the host and port with actual number you saw
```
./build/test/test_redis++ -h host -p port
```
If you see ```Pass all tests```, bingo!


Now, time to run an example, copy and paste the code, and name it as ```app.cpp```
```C++
...
```
Use ```g++ -std=c++17 -o app app.cpp /usr/lib/x86_64-linux-gnu/libredis++.a /usr/lib/libhiredis.a -pthread``` to compile it.
You will see an executable file ```app```
Use ```./app``` to run it, and you will see
```
val
m1 exists
```
printed on your screen. Congrats!!!!!
